...
func MachineInfo(nodeName string,
	...
	getCapacityForFlexVolumeFunc func() (string,error),
) Setter {
	return func(node *v1.Node) error {
		...
		if err != nil {
			...
			resourceStorage, err := getCapacityForFlexVolumeFunc()
			if err != nil {
				node.Status.Capacity[v1.ResourceStorage] = resource.MustParse("0Gi")
			} else {
				node.Status.Capacity[v1.ResourceStorage], err = resource.ParseQuantity(resourceStorage)
				if err != nil {
					node.Status.Capacity[v1.ResourceStorage] = resource.MustParse("0Gi")
				}
			}
